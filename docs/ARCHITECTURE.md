...
Both directors consume one `ModelFamilyPolicy` object, resolved once per session/worker from the provider/model via `detectModelFamily` (`src/subagent/provider-family.ts`) — the directors branch on this data, never on per-family subclasses or forks. `resolveModelFamilyPolicy({ providerName, model, orchestrator? })` returns:
...
**There is no automated stop for the primary session.** The exact-period tool-fingerprint thrash detector, the operator-facing hard-pause, and the turns-since-user-message backstop (with its worker-progress exemption for fleet-heavy work) that a prior release ran against this streak have all been removed, along with `src/util/period-detection.ts`. A primary session that loops on tool calls now gets the one-shot check-in nudge above and nothing further — it keeps running until the operator intervenes, a worker-level stop fires inside a dispatched fleet agent, or some other bound (provider error, `--agent-timeout-ms` in eval mode, an opt-in tool timeout) ends the turn.
...
Skills are Markdown capability packages (`SKILL.md`). Each skill is a **dual surface**: the model looks up descriptions with the advertised `skill_search` catalog tool, loads a body on demand via the `use_skill` core tool, and the operator can invoke it as `/<skill-name>` unless frontmatter sets `user-invocable: false`. The primary prompt lists names only. `loadSkillCommands` (`src/plugins/skill-commands.ts`) synthesizes a slash command that sends the SKILL.md body (plus typed args) to the primary session; untagged skills still become slashes.
...
`discoverSkills(cwd, pluginDirs)` runs at session start and returns each available skill's `name` + one-line `description`. The system prompt listing is names only; descriptions feed `skill_search` (and the slash picker). It scans the following base directories, highest precedence first:
...
| -------------------------- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `description`              | yes          | One-line summary shown in `skill_search` results and the slash picker — not the prompt listing                                                                                                                                                    |
